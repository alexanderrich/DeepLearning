        ...
        for f = 1, inputFrameSize do -- per feature column
            ...
            for j = 1, nOutputFrame do -- per output element
                ...
                for s = 1, kW do -- per subset element
                    ii = i+s-1
                    if (subset[s] == torch.max(subset)) then
                        gradInput[ii][f] = gradInput[ii][f] + (1 * gradOutput[j]) -- update gradInput
                    end
                end
            i = i + dW
            end
            ...
            for f = 1, inputFrameSize do -- per feature column
                ...
                for j = 1, nOutputFrame do -- per output element
                    ...
                    for s = 1, kW do -- per subset element
                        ii = i+s-1
                        if (subset[s] == torch.max(subset)) then
                            gradInput[b][ii][f] = gradInput[b][ii][f] + (1 * gradOutput[b][j]) -- update gradInput
                        end
                    end
                end
                i = i + dW
