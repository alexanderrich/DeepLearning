...
mean = {}
   for i = 1,table.getn(channels) do
      mean[i] = {}           -- create the matrix
      for x=1,N do
         mean[i][x] = {}    -- create a new row
         for y=1,M do
            mean[i][x][y] = 0
         end
      end
   end
std = {}          -- create the matrix
    for i = 1,table.getn(channels) do
      std[i] = {}           -- create the matrix
      for x=1,N do
         std[i][x] = {}    -- create a new row
         for y=1,M do
            std[i][x][y] = 0
         end
      end
...
for i,channel in ipairs(channels) do
   for x=1,M do 
      for y=1,N do
         ...
         mean[i][x][y] = trainData.data[{{}, i, x, y}]:mean()
         std[i][x][y] = trainData.data[{{}, i, x, y}]:std()
         trainData.data[{{}, i, x, y}]:add(-mean[i][x][y])
         trainData.data[{{}, i, x, y}]:div(std[i][x][y])
      end
   end
end
...
for i,channel in ipairs(channels) do
   for x=1,M do 
      for y=1,N do
         -- normalize each channel globally:
         testData.data[{{}, i, x, y}]:add(-mean[i][x][y])
         testData.data[{{}, i, x, y}]:div(std[i][x][y])
      end
   end
end
----------------------------------------------------------------------
...
for i,channel in ipairs(channels) do
   for x=1,M do 
      for y=1,N do
         trainMean = trainData.data[{ {},i, x, y }]:mean()
         trainStd = trainData.data[{ {},i, x, y }]:std()

         testMean = testData.data[{ {},i, x, y }]:mean()
         testStd = testData.data[{ {},i, x, y }]:std()

         trainMean1 = trainData.data[{{1}, i, {}, {}}]:mean()
         trainStd1 = trainData.data[{{1}, i, {}, {}}]:std()

         print('training data, '..channel..'-channel, mean: ' .. trainMean)
         print('training data, '..channel..'-channel, standard deviation: ' .. trainStd)

         print('training data1, '..channel..'-channel, mean: ' .. trainMean1)
         print('training data1, '..channel..'-channel, standard deviation: ' .. trainStd1)
